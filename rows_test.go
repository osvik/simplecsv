...
func TestSimpleCsv_UpdateRowCellsFromMap(t *testing.T) {
	type args struct {
		rowNumber int
		rowValue  map[string]string
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1 := tt.s.UpdateRowCellsFromMap(tt.args.rowNumber, tt.args.rowValue)
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("SimpleCsv.UpdateRowCellsFromMap() got = %v, want %v", got, tt.want)
			}
			if got1 != tt.want1 {
				t.Errorf("SimpleCsv.UpdateRowCellsFromMap() got1 = %v, want %v", got1, tt.want1)
			}
		})
	}
}
